...
# Compatibility-first template for cufflinks.
...
RUN set -eux; \
    BIN=""; \
    if [ -x "/opt/conda/bin/cufflinks" ]; then BIN="/opt/conda/bin/cufflinks"; fi; \
    if [ -z "$BIN" ]; then CAND="/opt/conda/bin/$(echo cufflinks | tr '-' '_')"; [ -x "$CAND" ] && BIN="$CAND" || true; fi; \
    if [ -z "$BIN" ]; then BIN="$(find /opt/conda/bin -maxdepth 1 -type f -perm -111 -name 'cufflinks*' | head -n1 || true)"; fi; \
    test -n "$BIN"; \
...
RUN set -eux; \
    ...
    printf '#!/usr/bin/env bash\nexec "%s" "$@"\n' "$BIN" > /usr/local/bin/cufflinks
RUN chmod +x /usr/local/bin/cufflinks && rm -f /tmp/tool-entry-path
WORKDIR /data
